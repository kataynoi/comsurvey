$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_use_status/fetch_admin_use_status',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/admin_use_status/del_admin_use_status',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/admin_use_status/save_admin_use_status',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/admin_use_status/get_admin_use_status',
                       params = {
                           id: id
                       }
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุชื่อสถานะการใช้งาน");$("#name").focus();}
    else{
        return true;
    }
