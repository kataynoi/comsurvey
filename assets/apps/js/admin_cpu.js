...
crud.set_update = function (data) {
    $("#id").val(data.rows["id"]);$("#name").val(data.rows["name"]);$("#cpu_series").val(data.rows["cpu_series"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.id=$("#id").val();items.name=$("#name").val();items.cpu_series=$("#cpu_series").val();
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุประเภทCPU");$("#name").focus();}else if (!items.cpu_series) { swal("กรุณาระบุรหัสย่อย CPU");$("#cpu_series").focus();}
    else{
        return true;
    }
